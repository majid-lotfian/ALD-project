...
def sample_world_hyperparams(rng: np.random.Generator) -> Dict:
    ...
    # V7: b_infl added — LPC/LPE species are released by phospholipase A2 during
    # neuroinflammation, so inflammation directly drives VLCFA_LPC accumulation.
    # This is the primary fix for the reversed ρ(VLCFA_LPC, severity) seen in V6.
    ...
    # V7: b_infl added — inflammation alters PC/PE remodelling (phospholipase A2
    # activity, Lands cycle perturbation). Conservative range to limit extra
    # within-class correlation without overshooting.
    ...
    # V7: b_infl negative — TNF-α and systemic inflammation suppress hepatic
    # lipoprotein/triglyceride production, decreasing TG/CE in severe ALD.
    # This moves synthetic ρ(TG_CE, severity) from ~0 toward the real ρ ≈ −0.30.
...
def compute_lipid_factors(roots, vlcfa, vlcfa_c24, oxidative_stress, infl, params, rng):
    ...
    # V7: added b_infl term — LPC/LPE are released by PLA2 during neuroinflammation
    p = lf_p["VLCFA_LPC"]
    L = (p["b0"]
         + p["b_vlcfa"]    * vlcfa
    ...
    # V7: added b_infl term — inflammation drives PC/PE remodelling via PLA2/Lands cycle
    p = lf_p["PC_PE"]
    L = (p["b0"]
         + p["b_vlcfa"] * vlcfa
         + p["b_infl"]  * infl
    ...
    # V7: added negative b_infl term — systemic inflammation suppresses hepatic
    # lipoprotein synthesis, reducing TG/CE in severe disease
    p = lf_p["TG_CE"]
    L = (p["b0"]
         + p["b_vlcfa"] * vlcfa
         + p["b_infl"]  * infl
...
def sample_world_like(
    ...
    V7 changes vs V6
    ----------------
    1. b_infl term (positive, U(0.3, 0.7)) added to VLCFA_LPC factor.
       Root cause of reversed ρ(VLCFA_LPC, severity) in V5/V6: the factor was
       driven only by vlcfa and oxidative_stress, both of which are age-confounded
       (a_age dominates V_star). Adding b_infl * infl creates a direct positive
       path  severity → inflammation → VLCFA_LPC lipid  that is not age-confounded.
       Biology: LPC/LPE species are released by phospholipase A2 during ALD
       neuroinflammation. Fixes all three reversed rows in Fig 6 (VLCFA/LPC,
       PC/PE, Plasmalogen) because all three representative lipids are VLCFA_LPC-
       driven in the causal model.

    2. b_infl term (positive, U(0.2, 0.5)) added to PC_PE factor.
       Conservative range chosen to fix true PC_/PE_ lipids without further
       inflating within-class Pearson correlations (Fig 3 concern).
       Biology: phospholipase A2 / Lands cycle remodelling during inflammation.

    3. b_infl term (negative, U(-0.4, -0.2)) added to TG_CE factor.
       Moves synthetic ρ(TG_CE, severity) from ~0 toward the real ρ ≈ −0.30.
       Biology: TNF-α and systemic inflammation suppress hepatic VLDL/TG synthesis.
       Negative sign also partially counteracts the over-inflation in between-class
       correlations (Fig 3), since TG/CE now anti-correlates with high-infl classes.

    Inherited from V6 (unchanged)
    ------------------------------
    - Per-column z-score calibration (Fix 1 of V6)
    - copula_noise_scale = 0.65
    - w_cross ~ N(0, 0.12)

    Causal order (identical to V5/V6)
    ----------------------------------
    ...
    # Per-column z-score calibration (V6 Fix 1, retained)
    for col in lipid_cols:
        v = out[col]
        sigma = v.std()
        if sigma > 1e-8:
...
if __name__ == "__main__":
    ...
    generate_many_worlds(
        ...
        n_samples          = N_SAMPLES,
        base_seed          = 42,
        out_dir            = "./synthetic_data/causal_knowledge_v7/",
        prefix             = "ALD_world",
        n_jobs             = N_JOBS,
        copula_components  = cop_comp,
        copula_variance    = cop_var,

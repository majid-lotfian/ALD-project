...
def sample_world_hyperparams(rng: np.random.Generator) -> Dict:
    ...
    params["lipid_factors"]["VLCFA_LPC"] = dict(
        ...
        b1       = rng.uniform(0.8, 1.8),   # strong VLCFA effect
    ...
    params["lipid_factors"]["TG_CE"] = dict(
        b0       = rng.normal(0.0, 0.5),
        b1       = rng.uniform(0.1, 0.8),   # weaker VLCFA
        b2       = rng.uniform(0.7, 1.7),   # stronger metabolic
        noise_sd = rng.uniform(0.1, 0.4),
    )
    ...
    # Severity (latent score + thresholds)
    params["severity"] = dict(
        e0         = rng.normal(0.0, 0.5),
        e_D        = rng.uniform(1.0, 2.5),
        e_I        = rng.uniform(0.5, 2.0),
        e_age      = rng.uniform(-0.5, 0.5),
        e_sex      = rng.uniform(0.3, 1.5),
        noise_sd_S = rng.uniform(0.3, 0.8),
        t1         = rng.normal(-1.0, 0.3),
        t2         = rng.normal( 0.0, 0.3),
        t3         = rng.normal( 1.0, 0.3),
    )
...
def sample_roots(n: int, rng: np.random.Generator) -> Dict[str, np.ndarray]:
    ...
    # Age mixture (children + adults); internal only
...
# ============================================================
# 5) Structural equations (ABCD1, VLCFA, factors, inflammation, demyelination, severity)
# ============================================================

def compute_abcd1_effective(
    roots: Dict[str, np.ndarray], params: Dict, rng: np.random.Generator
) -> np.ndarray:
    p = params["abcd1"]
...
def compute_severity(
    ...
    severity_cat = np.full(n, "control", dtype=object)
    severity_cat[(S_star >= t1) & (S_star < t2)] = "mild"
    severity_cat[(S_star >= t2) & (S_star < t3)] = "severe"
    severity_cat[S_star >= t3]                  = "CALD"

    return S_star, severity_cat


# ============================================================
# 6) Map real lipid column name → main latent factor
# ============================================================

def assign_main_factor(col: str) -> str:
    """
    Map a real lipid column name to one of the four latent factors.
    Adapt rules if you refine lipid class mapping.
    """

    # Lysophospholipids (LPC/LPE) → VLCFA_LPC factor
    if (
    ...
    # Fallback: treat as PC/PE-like
    return "PC_PE"


# ============================================================
# 7) Generate ONE synthetic world with the SAME schema as df_template
# ============================================================
...
def sample_world_like(
    ...
    """
    Generate one synthetic ALD cohort ("world") with:
      - exactly the same columns as df_template
      - internal latent variables used, but NOT exposed
    """
    rng = np.random.default_rng(cfg.seed)
    params = sample_world_hyperparams(rng)
    n = cfg.n_samples

    # ---- Internal causal generation ----
    ...
    S_star, severity_cat = compute_severity(roots, infl, demy, params, rng)

    # ---- Build output with EXACT same columns as template ----
    cols = list(df_template.columns)
    out = {}

    # We assume the first columns are meta: Sample_ID, sex, severity
    # (as in your ALD_lipidomics_merged.csv)
    ...
    if "sex" in cols:
        # 0/1 numeric encoding, consistent with your real dataset (0=female,1=male)
        out["sex"] = roots["sex_male"].astype(int)

    if "severity" in cols:
        # Map our categorical severity to an integer code 0..3
        # You can adjust mapping if you want to align with your real coding.
        cat_to_int = {
            "control": 0,
            "mild":    1,
            "severe":  2,
        ...
        sev_int = np.array([cat_to_int[c] for c in severity_cat], dtype=int)
        out["severity"] = sev_int

    # Lipid columns: one synthetic series per real lipid
...
# ============================================================
# 8) Generate MANY worlds and optionally save them
# ============================================================
...
def generate_many_worlds(
    ...
    """
    Generate n_worlds synthetic cohorts.
    Each world has the same column schema as df_template.

    If out_dir is provided, saves each world as:
      out_dir / f"{prefix}_{i:03d}.csv"
    """
...
if __name__ == "__main__":
    # Load your real merged dataset once
    real_path = "./data/ALD_lipidomics_merged.csv"
    df_real = pd.read_csv(real_path)

    # 1) Generate a single synthetic world with same size as real
    ...
    # 2) Generate many worlds (e.g., 10) and optionally save them
    worlds = generate_many_worlds(
        df_template=df_real,
        n_worlds=1,
        n_samples=2000,
        base_seed=100,
        out_dir="./synthetic_data/causal/",          # or a path like "synthetic_data"
        prefix="ALD_world"
    )

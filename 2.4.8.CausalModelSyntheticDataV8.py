...
# ============================================================
# 7b) Empirical marginals — VLCFA/LPC and Plasmalogen
# ============================================================

def fit_empirical_marginals(
    df_real: pd.DataFrame,
    meta_cols: Tuple[str, ...] = ("Sample_ID", "sex", "severity"),
    target_factors: Tuple[str, ...] = ("VLCFA_LPC", "PLASMALOGEN"),
) -> Dict[str, np.ndarray]:
    """Fit empirical quantile functions for VLCFA/LPC and Plasmalogen columns.

    For each qualifying column, stores the sorted real values (the empirical
    quantile function Q(u) = real_sorted[floor(u*(m-1))]).  Used in
    apply_empirical_marginals() to replace synthetic Gaussian marginals with
    the exact real marginal shape for these two worst-matching lipid classes.

    Returns
    -------
    dict  col -> np.ndarray of sorted real values (length = non-NaN count)
    """
    marginals: Dict[str, np.ndarray] = {}
    for col in df_real.columns:
        if col in meta_cols:
            continue
        if assign_main_factor(col) not in target_factors:
            continue
        vals = df_real[col].dropna().values.astype(float)
        if len(vals) < 5:
            continue
        marginals[col] = np.sort(vals)

    print(f"  Empirical marginals: fitted {len(marginals)} columns "
          f"(VLCFA/LPC + Plasmalogen)")
    return marginals


def _quantile_map(syn_vals: np.ndarray, real_sorted: np.ndarray) -> np.ndarray:
    """Map syn_vals through the empirical quantile function of real_sorted.

    Steps
    -----
    1. Dense-rank synthetic values -> uniform quantiles u in (0, 1).
    2. Linearly interpolate through sorted real values at positions u.

    Spearman correlation is invariant (monotone transform preserves ranks).
    Pearson correlation improves because the marginal shape now matches real.
    """
    m = len(real_sorted)
    ranks = np.argsort(np.argsort(syn_vals))          # dense rank, 0-indexed
    u = (ranks + 0.5) / len(syn_vals)                 # midpoint quantiles ∈ (0,1)
    return np.interp(u, np.linspace(0.0, 1.0, m), real_sorted)
...
def sample_world_like(
    ...
    copula_noise_scale: float = 0.65,
    marginals: Optional[Dict[str, np.ndarray]] = None,
) -> Tuple[pd.DataFrame, Dict]:
    """Generate one synthetic ALD cohort matching df_template's column schema.

    V8 changes vs V7
    ----------------
    Empirical marginal matching for VLCFA/LPC and Plasmalogen (Option 2).

    After the per-column z-score calibration step inherited from V6, the
    synthetic values for every VLCFA/LPC and Plasmalogen column are passed
    through the empirical quantile function fitted from the real data
    (see fit_empirical_marginals / _quantile_map).

    Mechanically this is a NORTA (Normal-To-Anything) / quantile-mapping
    transform:
      rank synthetic values -> uniform quantiles -> interpolate through
      sorted real values.

    Effect on each validation figure
    ---------------------------------
    Fig 1  KS annotations for VLCFA/LPC and Plasmalogen panels drop.
    Fig 2  No change (sex/severity columns not touched).
    Fig 3  Pearson correlations in the two blocks improve (marginals now
           match real; Spearman/copula structure is unchanged).
    Fig 4  VLCFA/LPC (0.109) and Plasmalogen (0.129) violins shift down.
    Fig 5  AUC moves closer to 0.50 (classifier can no longer exploit the
           marginal mismatch of the two hardest classes).
    Fig 6  No change (Spearman ρ is invariant under monotone transforms).
    Fig 7  Per-world means of representative lipids shift toward real means.

    Inherited unchanged from V7
    ---------------------------
    - b_infl for VLCFA_LPC, PC_PE, TG_CE factors
    - Per-column z-score calibration (V6)
    - copula_noise_scale = 0.65, w_cross ~ N(0, 0.12)
    - Gaussian copula correlated noise
    - Quantile-based severity thresholds, sex ratio calibration (V5)
    """
    ...
    if (copula_components is not None
        ...
        corr_noise = (z * np.sqrt(copula_variance)) @ copula_components
    ...
    # Per-column z-score calibration (V6, retained)
    for col in lipid_cols:
        v = out[col]
        sigma = v.std()
        if sigma > 1e-8:
            out[col] = (v - v.mean()) / sigma

    # V8: empirical marginal matching for VLCFA/LPC and Plasmalogen.
    # Applied after z-score so the Gaussian shape is well-defined before mapping.
    # Spearman ρ (Fig 6) is invariant; Pearson (Fig 3) and KS (Figs 1, 4) improve.
    if marginals:
        for col in lipid_cols:
            if col in marginals:
                out[col] = _quantile_map(out[col], marginals[col])
...
def _generate_one(args):
    (i, df_template, n_samples, seed, out_dir, prefix,
     cop_comp, cop_var, cop_cols, cop_scale, marginals) = args
...
if __name__ == "__main__":
    ...
    N_WORLDS = 500
    N_SAMPLES = 20_000
    N_COPULA_COMPONENTS = 30
    COPULA_NOISE_SCALE  = 0.65
    ...
    print(f"Fitting empirical marginals for VLCFA/LPC and Plasmalogen …")
    marginals = fit_empirical_marginals(df_real)
    ...
    generate_many_worlds(
        ...
        n_samples          = N_SAMPLES,
        base_seed          = 42,
        out_dir            = "./synthetic_data/causal_knowledge_v8/",
        prefix             = "ALD_world",
        n_jobs             = N_JOBS,
        copula_components  = cop_comp,
        copula_variance    = cop_var,

...
def compute_norm_stats_from_files(
    ...
    for file_idx, fp in enumerate(csv_files):
        if file_idx % 50 == 0:
            print(f'  norm_stats: {file_idx}/{n_files} files...', flush=True)
        rows_seen = 0
        reader = pd.read_csv(fp, usecols=lambda c: c in feature_set, chunksize=chunksize)
        for chunk in reader:

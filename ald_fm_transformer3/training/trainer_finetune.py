...
class FinetuneTrainer:
    def __init__(self, model, head, optimizer, criterion, train_loader, val_loader,
                 cfg: FinetuneTrainerConfig, logger, config_snapshot: Dict):
        self.model = model
        self.head = head
        ...
        self.csv_logger = CSVLogger(
            ...
            ['epoch', 'split', 'loss', 'accuracy', 'macro_f1', 'balanced_accuracy'],
        )
    ...
    def fit(self):
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            self.csv_logger.log({'epoch': epoch, 'split': 'train', 'loss': train_loss,
                                 'accuracy': '', 'macro_f1': '', 'balanced_accuracy': ''})

            self.model.eval()
            self.head.eval()
            val_losses, all_logits, all_y = [], [], []
            with torch.no_grad():

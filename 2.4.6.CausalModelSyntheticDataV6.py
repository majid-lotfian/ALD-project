...
def sample_world_like(
    ...
    V6 changes vs V5
    ----------------
    1. Per-column z-score calibration  — after all lipid values are assembled, each
       column is centred and scaled to mean=0, std=1 (matching the real z-scored data).
       Root cause fixed: relu(oxidative_stress) and relu(inflammation) in the causal
       equations create a systematic positive bias that shifts every world's lipid
       means to +0.5–+1.0 vs. real mean=0.  Linear transform preserves ALL rank-based
       statistics (Spearman ρ, within-class Pearson correlations, copula structure).
       Expected impact: KS ↓↓, AUC 0.923 → ~0.7, Fig-7 lipid means collapse to real.

    2. copula_noise_scale 0.80 → 0.65  — increases the causal signal's share of
       total variance, strengthening severity–lipid associations (fixing the attenuation
       seen in Fig 6 where synthetic ρ was near-zero while real ρ ≈ 0.4–0.6).

    3. w_cross N(0, 0.08) → N(0, 0.12)  — wider between-class factor cross-loadings
       let the causal between-class structure (SM↔Ceramide negative via nSMase,
       Plasmalogen↔VLCFA_LPC negative via ROS) propagate more strongly into individual
       lipids, sharpening the between-class correlation block boundaries (Fig 3).
    ...
    # V6 Fix 1: per-column z-score calibration ─────────────────────────────
    # Removes the systematic positive-mean bias introduced by relu(oxidative_stress)
    # and relu(inflammation) in the causal equations.  Every synthetic lipid column
    # is rescaled to mean=0, std=1, matching the real z-scored lipidomics data.
    # Rank order within each column is unchanged → Spearman correlations,
    # copula correlation structure, and severity associations are fully preserved.
    for col in lipid_cols:
        v = out[col]
        sigma = v.std()
        if sigma > 1e-8:
...
if __name__ == "__main__":
    ...
    COPULA_NOISE_SCALE  = 0.65  # reduced from V5's 0.80 → more causal signal share
    ...
    generate_many_worlds(
        ...
        n_samples          = N_SAMPLES,
        base_seed          = 42,
        out_dir            = "./synthetic_data/causal_knowledge_v6/",
        prefix             = "ALD_world",
        n_jobs             = N_JOBS,
        copula_components  = cop_comp,
        copula_variance    = cop_var,

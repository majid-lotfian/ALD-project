...
def sample_world_hyperparams(rng: np.random.Generator) -> Dict:
    ...
    # Lipid factors (6 factors replacing the previous 4)
    ...
    params["lipid_factors"]["CERAMIDE"] = dict(
        b0       = rng.normal(0.0, 0.5),
        b_vlcfa  = rng.uniform(0.7, 1.6),   # CerS2/CerS3 incorporate accumulated VLCFA
    ...
    # Inflammation (updated: VLCFA + LPC + ceramide + oxidative stress inputs)
    # LPC activates microglia via GPR132/TLR; ceramide via PP2A/JNK; ROS directly
    # Ref: Fourcade et al., Hum Mol Genet 2008 (PMID 18628207); Jaspers 2024
    ...
    # Demyelination (updated: VLCFA + inflammation + ceramide-mediated oligodendrocyte apoptosis)
    ...
    # Severity (6 levels; now includes adrenal CE pathway)
    params["severity"] = dict(
        e0         = rng.normal(0.0, 0.5),
        e_D        = rng.uniform(0.2, 0.8),
        e_I        = rng.uniform(0.1, 0.6),
        e_ce       = rng.uniform(0.1, 0.4),   # adrenal VLCFA-CE → adrenal insufficiency
...
def compute_lipid_factors(roots, vlcfa, oxidative_stress, infl, params, rng):
    """Compute 6 lipid latent factors with literature-grounded structural equations.

    Factors (vs. 4 in V1/V2):
      VLCFA_LPC  — lysophospholipid biomarkers (iPLA2-mediated)
      PC_PE      — standard glycerophospholipids
      PLASMALOGEN — ether phospholipids depleted by ROS (vinyl-ether antioxidant sink)
      CERAMIDE   — very-long-chain ceramides (CerS2/CerS3-mediated C24/C26 incorporation)
      SM         — sphingomyelin (inversely regulated by inflammation via nSMase)
      TG_CE      — neutral lipids
    """
...
def compute_inflammation(vlcfa, oxidative_stress, factors, params, rng):
    """Neuroinflammation driven by VLCFA, LPC (microglial GPR132/TLR), ceramide, and ROS.

    LPC 26:0 acts as a microglial activator alongside shorter-chain LPCs.
    Ceramide activates PP2A, JNK/SAPK → pro-inflammatory/apoptotic signaling.
    Ref: Fourcade et al., Hum Mol Genet 2008 (PMID 18628207)
    """
...
def compute_severity(roots, infl, demy, ce_vlcfa, params, rng):
    """6-level ordinal severity.

    Now includes adrenal VLCFA-CE pathway reflecting that adrenal insufficiency
    is an independent severity dimension in ALD (present in ~70% of males before
...
def assign_main_factor(col: str) -> str:
    ...
    Changes vs. V2:
    - PE_O_, PC_O_ (ether bonds) → PLASMALOGEN  (depleted by ROS)
    - Cer_, HexCer_, Hex2Cer_ → CERAMIDE       (VLCFA-containing, CerS2/CerS3)
    - SM_, SM4_               → SM             (inversely regulated by inflammation)
    - C1P_, S1P_              → CERAMIDE        (ceramide metabolites)
    """
...
def sample_world_like(
    ...
    Causal order:
      roots → abcd1_effective → vlcfa (+ vlcfa_c24)
           ...
           → ce_vlcfa → demyelination → severity
    """
    ...
    vlcfa_c24  = compute_vlcfa_c24(roots, abcd1, params, rng)  # noqa: F841 — available if needed
    ...
    factors        = compute_lipid_factors(roots, vlcfa, ox_stress, infl_boot, params, rng)
    ...
    factors   = compute_lipid_factors(roots, vlcfa, ox_stress, infl, params, rng)
    infl      = compute_inflammation(vlcfa, ox_stress, factors, params, rng)

    ce_vlcfa = compute_ce_vlcfa(roots, vlcfa, params, rng)
    demy     = compute_demyelination(vlcfa, infl, factors, params, rng)
    S_star, severity = compute_severity(roots, infl, demy, ce_vlcfa, params, rng)
...
def generate_many_worlds(
    ...
    for i in range(n_worlds):
        print(f"World {i + 1}/{n_worlds} generated")
        seed = base_seed + i
...
if __name__ == "__main__":
    ...
    worlds = generate_many_worlds(
        ...
        n_samples   = 20_000,
        base_seed   = 42,
        out_dir     = "./synthetic_data/causal_knowledge_v3/",
        prefix      = "ALD_world",
    )
